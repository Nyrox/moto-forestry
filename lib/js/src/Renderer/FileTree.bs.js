...
var Tea_mouse = require("bucklescript-tea/lib/js/src-ocaml/tea_mouse.js");
...
function findFiles(filter, node) {
  if (node.tag) {
    return List.concat(List.map((function (param) {
                      return findFiles(filter, param);
                    }), node[0][1]));
  }
  var p = node[0];
  if (Curry._1(filter, p)) {
    return /* :: */[
            p,
            /* [] */0
          ];
...
function buildTree(path) {
  var a = readdir(Utils.Path.asString(path));
  return a.then((function (f) {
                var a = Promise.all($$Array.map((function (file) {
                            var path$1 = Utils.Path.extend(path, file);
                            var a = mapFileToStats(Utils.Path.asString(path$1));
...
function subscriptions(model) {
  var match = model.contextMenu;
  if (match !== undefined) {
    return Tea_sub.batch(/* :: */[
                Tea_mouse.ups(undefined, (function (param) {
                        return /* DestroyContextMenu */Block.__(2, [undefined]);
                      })),
                /* [] */0
              ]);
  } else {
...
function update(model, msg) {
  ...
  switch (msg.tag | 0) {
    ...
    case /* DestroyContextMenu */2 :
        var msg$1 = msg[0];
        var _model = msg$1 !== undefined ? update(model, msg$1) : model;
        return {
...
function dirEntry(replace, param) {
  ...
  return ContextMenu.contextMenuContainer(/* CreateContextMenu */Block.__(1, [contextMenu]), String(treeIteration.contents), (function (eta) {
              ...
              icon,
              /* :: */[
                Tea_html.text(Utils.Path.basename(name)),
                /* [] */0
              ]
            ]);
}
  ...
  if (!node.tag) {
    return /* :: */[
            Tea_html.li(undefined, undefined, /* :: */[
                  ...
                  /* :: */[
                    Vdom.onCB("dragstart", "", (function (e) {
                            ((e.dataTransfer.setData("application/elem-type", "file")));
                            
                          })),
                    /* [] */0
                  ]
                ], /* :: */[
                  Tea_html.text(Utils.Path.basename(node[0])),
                  /* [] */0
                ]),
            /* [] */0
          ];
